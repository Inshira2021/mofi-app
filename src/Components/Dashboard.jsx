...
const PopularMoviesSidebar = ({
  ...
  <aside className="hidden lg:block w-80 bg-gray-900 border-l border-gray-800 flex-shrink-0 overflow-y-auto p-4">
    {/* 🔍 Search bar has been completely removed */}
      ...
      <div className="space-y-4">
        ...
        {showPopularSeeMore && (
          <button
            ...
            className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-2 rounded-full mt-4"
          >
            See More
          </button>
        )}
      ...
      <div className="relative w-full h-96 bg-gray-700 rounded-lg overflow-hidden mb-10">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-gray-900 to-transparent p-8 flex flex-col justify-end">
          ...
          <div className="flex items-center space-x-4">
            <button className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-8 rounded-full flex items-center">
              <FiClock className="inline-block mr-2" /> Watch
            </button>
            <button className="bg-gray-700 hover:bg-gray-600 text-white font-bold py-3 px-8 rounded-full">
              + Add to List
            </button>
          </div>
        </div>
...
const ArtistCard = ({ name, movies, src }) => (
  <div className="bg-gray-900 rounded-2xl p-4 flex flex-col items-center shadow-md transition hover:scale-105">
    <img src={src} alt={name} className="w-28 h-28 object-cover rounded-full border-4 border-red-600" />
    <p className="font-semibold text-center mt-3 text-white text-base">{name}</p>
    ...
    <section className="mb-10 pr-4 text-white">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-3">
          <button onClick={handlePrev} disabled={currentPage === 0} className={`p-2 rounded-full transition ${currentPage === 0 ? 'text-gray-600 cursor-not-allowed' : 'bg-gray-700 hover:bg-gray-600'}`}>
            <FiChevronLeft className="w-5 h-5" />
          </button>
          <button onClick={handleNext} disabled={currentPage === totalPages - 1} className={`p-2 rounded-full transition ${currentPage === totalPages - 1 ? 'text-gray-600 cursor-not-allowed' : 'bg-gray-700 hover:bg-gray-600'}`}>
